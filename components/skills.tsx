                  ...
                  <div key={category.category} className="glass rounded-2xl p-5 transition-all duration-300 group">
                    ...
                    <div className="flex flex-wrap gap-1.5">
                      {category.skills.map((skill) => (
                        <span
                          key={skill}
                          className="px-2.5 py-1 text-xs rounded-lg border border-border bg-muted/50 text-muted-foreground hover:text-purple-500 hover:border-purple-500/30 hover:bg-purple-500/10 transition-colors cursor-default"
                        >
                          {skill}
                        </span>
                      ))}
            ...
            <div className="glass rounded-2xl p-6 transition-all duration-300">
              ...
              <div className="flex flex-wrap gap-2">
                {personalSkills.map((skill) => (
                  <span
                    key={skill}
                    className="px-3 py-1.5 text-xs rounded-full border border-purple-500/20 bg-purple-500/8 text-purple-500 dark:text-purple-400 font-medium"
                  >
                    {skill}
                  </span>
                ))}
              ...
              <div className="space-y-3">
                {languages.map((lang) => (
                  <div key={lang.language} className="flex items-center justify-between p-3 rounded-xl border border-border bg-muted/30">
                    ...
                    <span className="px-2.5 py-1 text-xs rounded-full border border-purple-500/25 bg-purple-500/10 text-purple-500 dark:text-purple-400 font-medium">
                      {lang.level}
                    </span>
                  </div>
                ))}
                ...
                >
                  <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-3 mb-4">
                    <div>
                      ...
                      <p className="text-purple-500 dark:text-purple-400 text-sm mt-0.5">{exp.organization}</p>
                    </div>
                  ...
                  <ul className="space-y-2">
                    {exp.points.map((point, i) => (
                      <li key={i} className="flex gap-3 text-sm text-zinc-400 leading-relaxed">
                        <span className="shrink-0 mt-1.5 w-1.5 h-1.5 rounded-full bg-purple-500" />
                        {point}
                      </li>
                    ))}
                  </ul>
